...
class WebCrawlFromPeople(object):
    '''从人民网冬奥会新闻列表页 'http://sports.people.com.cn/GB/419056/index1.html' 抓取新闻数据.
    ...
    def getUrlInfo(self, url):
        ...
        dtext = date_info.find('div', attrs={'class': 'fl'}).get_text()

        if len(dtext) > 16:
            date_source_str = dtext[0:16]
            temp_date = datetime.datetime.strptime(date_source_str , "%Y年%m月%d日%H:%M")
            date = datetime.datetime.strftime(temp_date, "%Y-%m-%d %H:%M:%S")
            print(date)
    ...
    def CrawlHistoryOlympicsNews(self, startPage, endPage):
        ...
        if AddressLst == []:
            urls = []
            url_Part_1 = 'http://sports.people.com.cn/GB/419056/index'
            url_Part_2 = '.html'
    ...
    def craw_and_storage(self,url):
        ...
        for strong in strong_list:
            ...
            if 'href' in a.attrs and a.string:
                summary, keyWords, date, article = self.getUrlInfo("http://sports.people.com.cn" + a['href'])
                if article != '':
                    data = {'date': date,
                            'address': "http://sports.people.com.cn" + a['href'],
                            'title': a.string,
                            'keywords': keyWords,
                            'summary': summary,
                            'article': article,
...
if __name__ == '__main__':
    ...
    crawler = WebCrawlFromPeople(total_page, split_page, ThreadsNum=4, IP="localhost", PORT=27017, \
